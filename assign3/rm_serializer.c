...
deserializeRecords(Schema *schema, char *recordStr, int sizeRecord) 
{
    ...
    while(token != NULL) {
		...
		char data[recordSize + 1];
		int offset = 14;
		// int attrStart = token + 14;
		for(int i = 0; i < schema->numAttr; i++) 
		{
			int result = 0;
			attrOffset (schema, i, &result);
			if(schema->dataTypes[i] == DT_INT || schema->dataTypes[i] == DT_BOOL) {
				strncpy(data + result, token + offset, 4);
				offset = offset + 3 + 2 + 1 + strlen(schema->attrNames[i]);
			} else if(schema->dataTypes[i] == DT_STRING) {
				strncpy(data + result, token + offset, schema->typeLength[i]);
				offset = offset + schema->typeLength[i] + 2 + strlen(schema->attrNames[i]);
			} 
		}
		data[recordSize] = '\0';
