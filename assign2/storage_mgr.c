// This file implements all interfaces listed in 'storage_mgr.h'
...
// The createPageFile function is to create a new page file with one PAGE_SIZE.
...
RC createPageFile(char *fileName) {
  ...
  // allocates memory for an array of PAGE_SIZE char and initialize them to '\0'
  ...
  // makes the page file fills with '\0' bytes
  int sizeNum = fwrite(str, sizeof(char), PAGE_SIZE, fp);
  if(sizeNum != PAGE_SIZE) {
    rc = RC_WRITE_PAGE_FAILED;
  }

  // closes file, flushes buffers, free the allocated memory
  fclose(fp);
  free(str);
...
RC openPageFile(char *fileName, SM_FileHandle *fHandle) {
  ...
  // check whether the opened file exists
  ...
  // initialize information about the opened file
  fHandle->mgmtInfo = fp;
...
RC readBlock(int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage) {
  ...
  // read data from disk to memory, update page
...
RC writeBlock(int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage) {
  ...
  if (fwrite(memPage, sizeof(char), strlen(memPage), fp) !=  strlen(memPage)) {
    return RC_WRITE_PAGE_FAILED;
  }
